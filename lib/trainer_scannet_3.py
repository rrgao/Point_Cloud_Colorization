    ...
    def _train_epoch(self, epoch):
        ...
        for curr_iter in range(len(data_loader) // iter_size): #0-5
            ...
            for iter_idx in range(iter_size): #1
                ...
                print(F0.size())
                rgb_ = torch.cat(input_dict['rgb'], input_dict['rgb']).cuda()
                diff_main = torch.unsqueeze(torch.max(torch.abs(F0[:, :, 0:3] - rgb_) / (rgb_ + 0.1), 1)[0], 2)
                diff_minor = torch.unsqueeze(torch.max(torch.abs(F0[:, :, 3:6] - rgb_) / (rgb_ + 0.1), 1)[0], 2)
                conf_map = torch.lt(diff_minor, diff_main).int()
                loss = torch.mean(torch.abs(F0[:, :3] - rgb_) * conf_map) + torch.mean(torch.abs(F0[:, :, 3:6] - rgb_) * (-conf_map + 1.0))
                loss /= iter_size
                loss.backward()
