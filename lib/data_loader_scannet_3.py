...
from config_scannet_3 import get_config
import os
...
class TrainDataset(SDataset):
  ...
  def __init__(self,
    ...
    for k in range(100):
      temp = np.loadtxt(os.path.join(self.DATA_DIRS[phase], list_file[k]), delimiter=' ', dtype='float32')
      temp1 = np.loadtxt(os.path.join("/disk1/rongrong/scan/semantic_0.025/", list_file[k]), delimiter=' ', dtype='float32')
