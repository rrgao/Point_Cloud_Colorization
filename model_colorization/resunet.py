...
from model_colorization.common import get_norm
from model_colorization.residual_block import get_block
...
class ResUNet2(ME.MinkowskiNetwork):
  ...
  def forward(self, x):
    ...
    # # v0.4
    #     # 对dim=1维求p=2范数，也就是有多少行则返回多少norm值
    #     # if dimension of F is [x, y, z] then return of torch.norm(F, p=2, dim=1, keepdim=True) is of dimension [x, 1,  z] or [x, z] for keepdim=False
    #    return ME.SparseTensor(out.F/torch.norm(out.F, p=2, dim=1, keepdim=True), coords= out.coords)
    #
    #else:
    #   return out
    return out
